...
export const createRepository = new Tool({
  id: "createRepository",
  ...
  execute: async ({ context }) => {
    const { name, description, private: isPrivate } = context;
    try {
...
export const forkRepository = new Tool({
  id: "forkRepository",
  ...
  execute: async ({ context }) => {
    ...
    try {
      const response = await octokit.request("POST /repos/{owner}/{repo}/forks", { owner, repo });
      return { ok: true as const, url: response.data.html_url };
    } catch (error) {
      return { ok: false as const, message: error instanceof Error ? error.message : "Unknown error" };
    }
